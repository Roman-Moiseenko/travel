...
class ReportController extends Controller
{
    ...
    public function __construct($id, $module, BookingCarRepository $bookings, $config = [])
    {
    ...
    public function actionIndex($id)
    {
        ...
        if ($form->confirmation) {
            $data = $this->bookings->getforChart($car->id, $form->month, $form->year, BookingHelper::BOOKING_STATUS_CONFIRMATION);
            $datasets[] =  $this->getDataset($data, 'blue', 'Подтверждено');
        }

        if ($form->booking) {
            $data = $this->bookings->getforChart($car->id, $form->month, $form->year, null);
            $datasets[] =  $this->getDataset($data, 'red', 'Забронировано');
        }
        if ($form->pay) {
            $data = $this->bookings->getforChart($car->id, $form->month, $form->year, BookingHelper::BOOKING_STATUS_PAY);
