...
/* @var $quantity int */
?>
<div id="data-day" data-d="<?= $D ?>" data-m="<?= $M ?>" data-y="<?= $Y ?>" data-count-times="<?= count($day_funs) ?>"></div>
<div class="row">
...
<table>
    ...
    <?php foreach ($day_funs as $i => $time): ?>
        <tr>
            <td>
                <input type="checkbox" class="form-control form-control-sm checkbox-time" id="checkbox-<?= $i ?>"
                       data-i="<?= $i ?>" <?= $time['check'] ? 'checked' : ''?>>
            </td>
            <td>
                <span class="badge badge-success" style="font-size: 16px"><?= $time['begin'] ?></span>
                <?= $time['end']  != null ? (' - <span class="badge badge-success" style="font-size: 16px">' . $time['end'] . '</span>') : '' ?>
                <input type="hidden" id="begin-<?= $i ?>" value="<?= $time['begin'] ?>">
                <input type="hidden" id="end-<?= $i ?>" value="<?= $time['end'] ?? null ?>">
            </td>
            <td>
            ...
            <td>
                <input type="number" class="form-control form-control-sm" min="0" value="<?= $time['cost_adult'] ?>"
                       id="cost-adult-<?= $i ?>">
            </td>
            <td>
                <?php if ($time['cost_child'] != null): ?>
                    <input type="number" class="form-control form-control-sm" min="0" value="<?= $time['cost_child'] ?>"
                           id="cost-child-<?= $i ?>">
                <?php endif; ?>
            </td>
            <td>
                <?php if ($time['cost_preference'] != null): ?>
                    <input type="number" class="form-control form-control-sm" min="0" value="<?= $time['cost_preference'] ?>"
