 ...
 * @property BookingAddressForm $endAddress
...
class ToursParamsForm extends CompositeForm
{
    ...
    public function __construct(ToursParams $params = null, $config = [])
    {
        if ($params != null)
        {
            ...
            $this->endAddress = new BookingAddressForm($params->endAddress);
            $this->ageLimit = new AgeLimitForm($params->agelimit);
        } else {
            $this->beginAddress = new BookingAddressForm();
            $this->endAddress = new BookingAddressForm();
    ...
    {
        return ['beginAddress', 'endAddress', 'ageLimit'];
    }
}