...
$js = <<<JS
    ...
    $(document).on('change', '#selling-calendar', function () {
            ...
            if (_date === "") {
                ...
                $.post('/selling-tour/get-time', {date: _date, tour_id: tour_id },
                    function (data) {
    ...
    $(document).on('change', '#selling-tour-time', function() {
        ...
        if (calendar_id == -1) {
            ...
            $.post('/selling-tour/get-selling', {calendar_id: calendar_id},
                function (data) {
    ...
    $(document).on('click', '#selling-add', function() {
        ...
        $.post('/selling-tour/add-selling', {calendar_id: calendar_id, count: _count},
            function (data) {
            $('#list-count').html(data);
        });      
    });
    $(document).on('click', '#selling-remove', function() {
        ...
        let calendar_id  = $('#selling-tour-time').val();
        $.post('/selling-tour/remove-selling', {selling_id: selling_id, calendar_id: calendar_id},
            function (data) {
                $('#list-count').html(data);
        });      
    });
...
<div class="d-flex p-2 justify-content-center" style="background-color: #85c17c; color: #134b18; font-size: 26px; font-weight: 600; text-align:  center;">
    <?= '<a class="link-head" href="' . Url::to(['/give/view', 'id' => $object_id]). '"><i class="fas fa-reply"></i></a>&#160;' . $tour->name ?>
</div>
