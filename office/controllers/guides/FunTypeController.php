...
use booking\services\office\guides\TypeFunService;
use yii\filters\AccessControl;
use yii\filters\VerbFilter;
use yii\web\Controller;

class FunTypeController extends Controller
{

    private $service;

    public function __construct($id, $module, TypeFunService $service, $config = [])
    {
    ...
    {
        $form = new FunTypeForm();
        if ($form->load(\Yii::$app->request->post()) && $form->validate()) {
            try {
                $this->service->create($form);
                return $this->redirect(['guides/fun-type/index']);
    ...
    public function actionAddCharacteristic($id)
    {
        ...
        if ($form->load(\Yii::$app->request->post()) && $form->validate()) {
            try {
                ...
                return $this->redirect(['guides/fun-type/view', 'id' => $type->id]);
            } catch (\DomainException $e) {
                \Yii::$app->errorHandler->logException($e);
                \Yii::$app->session->setFlash('error', $e->getMessage());
            }
    ...
    public function actionUpdateCharacteristic($id)
    {
        ...
        $form = new CharacteristicForm($characteristic->type_fun_id, $characteristic);
        if ($form->load(\Yii::$app->request->post()) && $form->validate()) {
            try {
                $this->service->updateCharacteristic($characteristic->type_fun_id, $characteristic->id, $form);
                return $this->redirect(['guides/fun-type/view', 'id' => $characteristic->type_fun_id]);
            } catch (\DomainException $e) {
                \Yii::$app->errorHandler->logException($e);
                \Yii::$app->session->setFlash('error', $e->getMessage());
            }
    ...
    {
        $characteristic = Characteristic::findOne($id);
        $this->service->removeCharacteristic($characteristic->type_fun_id, $characteristic->id);
        return $this->redirect(['guides/fun-type/view', 'id' => $characteristic->type_fun_id]);
    }
