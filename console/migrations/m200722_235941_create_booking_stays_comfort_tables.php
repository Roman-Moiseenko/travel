...
class m200722_235941_create_booking_stays_comfort_tables extends Migration
{
    ...
    public function safeUp()
    {
        ...
        $this->createIndex('{{%idx-booking_stays_assign_comfort-stays_id}}', '{{%booking_stays_assign_comfort}}', 'stays_id');
        $this->createIndex('{{%idx-booking_stays_assign_comfort-comfort_id}}', '{{%booking_stays_assign_comfort}}', 'comfort_id');

        $this->addForeignKey('{{%fk-booking_stays_assign_comfort-stays_id}}', '{{%booking_stays_assign_comfort}}', 'stays_id', '{{%booking_stays}}', 'id', 'CASCADE', 'RESTRICT');
        $this->addForeignKey('{{%fk-booking_stays_assign_comfort-comfort_id}}', '{{%booking_stays_assign_comfort}}', 'comfort_id', '{{%booking_stays_comfort}}', 'id', 'CASCADE', 'RESTRICT');
