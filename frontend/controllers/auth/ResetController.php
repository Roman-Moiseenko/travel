...
class ResetController extends Controller
{
    ...
    public function actionRequest()
    {
        ...
        if ($form->load(Yii::$app->request->post()) && $form->validate()) {
            ...
            try {
                Yii::$app->session->setFlash('success', Lang::t('Проверьте вашу почту, ключ отправлен'));
                return $this->goHome();
            } catch (\DomainException $e) {
                Yii::$app->session->setFlash('error', Lang::t('Пользователь не найден'));
            } catch (\RuntimeException $e) {
                Yii::$app->session->setFlash('error', Lang::t('Ошибка отправки почты'));
            }
        }
    ...
    public function actionReset($token)
    {
        ...
        if ($form->load(Yii::$app->request->post()) && $form->validate()) {
            try {
                ...
                Yii::$app->session->setFlash('success', Lang::t('Новый пароль сохранен'));
                return $this->goHome();
            } catch (\DomainException $e) {
    ...
    public function actionVerifyEmail($token)
    {
        ...
        try {
            ...
            if (Yii::$app->user->login($user)) {
                Yii::$app->session->setFlash('success', Lang::t('Ваша почта подтверждена!'));
                return $this->goHome();
            } {
                throw new \DomainException(Lang::t('Ошибка входа в систему'));
            }
        } catch (\DomainException $e)
        {
            Yii::$app->session->setFlash('error', $e->getMessage());
    ...
    public function actionResend()
    {
        ...
        if ($form->load(Yii::$app->request->post()) && $form->validate()) {
            try {
                ...
                Yii::$app->session->setFlash('success', Lang::t('Проверьте почту, мы Вам выслали инструкцию дальнейших действий'));
                return $this->goHome();
            } catch (\RuntimeException $e)
            {
                Yii::$app->session->setFlash('error', $e->getMessage());
